...
export default function Navbar({className}:{className?:string}) {
  ...
  return (
      ...
      >
        <h1 className='text-3xl font-bold underline underline-offset-8 decoration-green-500 -rotate-2'>ᯓ Yunus</h1>

        <div className='flex items-center gap-8'>
            {socials.map((social, index)=>{
                const Icon = social.Icon;
                return <Link target="_blank" href={social.link} key={index} aria-label={social.label}>
                    <Icon className='size-8 hover:scale-125 transition-all' />
                </Link>
            })}
        </div>
    </nav>
